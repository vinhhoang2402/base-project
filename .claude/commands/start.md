...
- New task  → write to `.claude/tasks/<ID>.md`
- Fix bug   → write to `.claude/bugs/<ID>.md`
...
2. Explore the relevant module(s) to understand the existing architecture and
   ...
   `~/.claude/skills/` (MVI, repository pattern, error handling, security, etc.).
...
4. As you go, keep `.claude/tasks/<ID>.md` (or `bugs/<ID>.md`) updated — tick off
...
Also remind them: the file `.claude/tasks/<ID>.md` (or bugs/) is there to update
...
- When they paste the hashes, treat them as ONE combined change set and run the
  ...
  2. Read the checklist: `~/.claude/skills/android-review/references/checklist.md`.
...
- Never invent commands — only use the real ones listed in `.claude/USAGE_GUIDE.md`.
