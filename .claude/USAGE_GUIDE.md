...
> Project command files (in `.claude/commands/`, also invocable as slash
> commands): `/start`, `/self-review`.
...
> follow them (e.g. "follow `.claude/skills/04-mvi-pattern.md`") or read the file.
...
- `.claude/skills/01…17-*.md` → coding conventions (ViewBinding, MVI, Moshi, …)
- `.claude/skills/android-review/` → Android review checklist + agent
...
Markdown docs under `.claude/skills/` — point Claude at them by name.
...
**Reviewing a diff:**
```bash
cat .claude/skills/android-review/references/checklist.md   # read checklist
